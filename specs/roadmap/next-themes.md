...
| 2026-04-30 | **Theme B — B1 (Lead → Sale funnel)** | **Shipped, then merged into Sales module per user direction (2026-04-30)** | First shipped as a standalone `/leads` module (SPEC-LEADS-001 v1.0 + 22 tests). User feedback: "remove the separate lead module — sales already handles everything." Lead lifecycle is now modelled by `Deal` (sales.ts) with stages `'new-lead' / 'contacted' / 'test-drive' / 'reserved' / 'sales-order' / 'delivered' / 'lost' / 'on-hold'`. The `/leads` route + `useLeadsStore` + `Lead` type + leads fixture are DELETED. Service-upgrade trigger (B4) calls `useSalesDealsStore.getState().createLeadFromService(...)`. SPEC-LEADS-001 marked deprecated. |
