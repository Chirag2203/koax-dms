...
| 2026-04-20 | **Phase E shipped** — Confidentiality enforcement pass complete. New `ConfidentialToggle` component in Customer 360 Profile tab (R19+ Gate-gated edit; toggle calls `customers.updateCustomerProfile`). `maskedContactFor` wired into 8 components: customers-table, customer-360-header, customer-profile-tab, claim-review-panel, jobcard-detail-view, appointment-detail-view, appointment-checkin-dialog, plus the toggle itself. Lock icon affordance with tooltip "Confidential — full contact requires R19+ access". Grep audit clean: zero raw `customer.phone`/`customer.email` reads in display code (only Boolean guards, URL hrefs, and the R19+-gated type-to-confirm in Right-to-Erasure dialog remain raw — all intentional). **Bug fix during this phase**: `R19_RANK` constant in `@dms/vehicles-core/contact-mask` was 4 (would have let R12 bypass masking) — corrected to 6 to match `ROLE_RANK['R19']` in state-machine. Smoke verified: at R24 Karan Shah's contact shows raw values; at R09 BLR same page shows `+91 *** *** **02` and `k***@***.com`. |
| 2026-04-20 | **PLAN-VEHICLES-002 fully shipped** — all 5 phases (A schema + helper + fixture audit, B hydrator backfill, C sale-inventory rename + Add Car flow, D dashboard claims card, E confidentiality enforcement) landed. Ready for v2 follow-ups: real inventory store wiring (`createListing`), v2 fixture cleanup of Z-bearing VINs, customer-side `contactConfidential` portal toggle (L13), real backend saga compensation for partial-failure flows. |
| 2026-04-20 | **Polish pass** (per user feedback — data must feel real): (1) Inventory backfill now generates a **2-row ownership chain** per VIN — a prior consignor (deterministically picked by VIN-hash from the city-matched customer pool, excluding canonical demo owners) TRANSFERRED on a realistic `acquiredAt = listedAt − 15–45 days`, plus the BN_CONSIGNMENT ACTIVE row. Prior tenure synthesized 2–5 years deep with plausible km progression. (2) Removed all synthetic markers from UI surfaces: `system-auto-backfill` actor resolved to "BN Automobiles"; `meta.backfill` keys stripped from event payloads (filter applied in renderer for any legacy events); `OwnershipEvent.payload` values formatted via typed `PAYLOAD_LABELS` + `SOURCE_LABELS` + `CLOSE_REASON_LABELS` maps — zero `[object Object]` in DOM. (3) Source strings display as title-cased ("BN Consignment" not "BN CONSIGNMENT") in both Ownership tab and timeline events. Date format locked to `dd MMM yyyy`. Actor ids resolved via `STAFF_NAMES` registry → customer-store fallback → "BN Automobiles" default. (4) Add Car flow pages refactored from centered 640px card to the canonical `max-w-[1440px] px-6 pb-12 pt-6` container + `28px` h1 + `border-b border-line pb-5 mb-8` divider used by the rest of staff-web. Tests: 15/15 backfill tests still pass (one assertion updated to reflect clean-payload contract). |
